...
type RoutingRepository interface {
	...
	CreateRoutingRule(ctx context.Context, teamID uuid.UUID, matchLabels map[string]string, priority int32) (db.RoutingRule, error)
	UpdateRoutingRule(ctx context.Context, id, teamID uuid.UUID, matchLabels map[string]string, priority int32) (db.RoutingRule, error)
...
func (s *RoutingService) CreateRule(ctx context.Context, teamID uuid.UUID, matchLabels map[string]string, priority int32) (db.RoutingRule, error) {
	if err := s.validateRule(ctx, teamID, matchLabels); err != nil {
		return db.RoutingRule{}, err
	}
	rule, err := s.repo.CreateRoutingRule(ctx, teamID, matchLabels, priority)
	if err != nil {
...
func (s *RoutingService) UpdateRule(ctx context.Context, id, teamID uuid.UUID, matchLabels map[string]string, priority int32) (db.RoutingRule, error) {
	if err := s.validateRule(ctx, teamID, matchLabels); err != nil {
		return db.RoutingRule{}, err
	}
	rule, err := s.repo.UpdateRoutingRule(ctx, id, teamID, matchLabels, priority)
	if err != nil {
		return db.RoutingRule{}, mapRoutingError(err)
	}
	return rule, nil
...
func (s *RoutingService) validateRule(ctx context.Context, teamID uuid.UUID, matchLabels map[string]string) error {
