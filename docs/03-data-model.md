...
**teams:** `id`, `name` (unique), `description`, `workspace_id`, `support_tier`, `express_chat_id`, `slack_channel_id`, `slack_user_group_id`, `oncall_announced_user_ids`, `created_at`, `updated_at`.

`slack_channel_id` and optional `slack_user_group_id` are the per-team Slack destination settings used to publish who is on call. `express_chat_id` remains stored for rollback compatibility but is not used for on-call publication; eXpress uses the global `oncall_group_chat_id` integration setting instead. `oncall_announced_user_ids` stores the last published on-call user-id fingerprint for rotation change detection.
